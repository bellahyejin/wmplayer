		...
		<div class="member-table">
			<div class="member-subject">
				...
				<div class="member-filter">
					...
					<input type="radio" name="gender" class="query-radio" value="all" />전체
		...
		if (!JSONDataCompare(this.prev_data, this.data))
		{
			...
			$.ajax({
				...
				success : function(data, status, xhr)
				{
					...
					$(".pager-container-manager").html(temp[1] == undefined || temp[1] == null ? "" : temp[1]);
				},
	...
	$(document).ready(function()
		{
			...
			gender = "${ param.gender }" == "" ? "all" : "${ param.gender}";
			search = "${ param.search }" == "" ? "userid" : "${ param.search }";
			value = "${ param.value }";

			$("#query-status-select option[value='" + status + "']").prop("selected", true);
			$(".query-radio[value='" + gender + "']").prop("checked", true)
			$("#query-search-select option[value='" + search + "']").prop("selected", true);
			$("#query-text").val(value);
					...
					$("#query-text").keyup(function(e)
						{
							...
							if (key == 13 || key == 8 && $(this).val() == "") ajaxStart();
						});
