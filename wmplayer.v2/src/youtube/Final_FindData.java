...
public class Final_FindData {
	...
	public Map<String, Object> contents_replace(Map<String, Object> map,
			Map<String, Boolean> key_name_list) {
	...
	public Final_FindData() throws InterruptedException, IOException {
		int year = 2010; // ���� ����(?) �⵵
		int file_num = 0; // ���������� ���������� ���� "�����ڵ� ���� complete"���� ���ڸ� �����ϸ��
		int max_print = 30; // ���� �ʹ� ���� ��������� ���� �÷��� ��, ��� �������� �׸�ŭ �ð��� �� ����
		...
		for (int start_idx : start_index_list) {
			url = "http://developer.echonest.com/api/v4/artist/search?api_key="
					+ echonest_key
					+ "&format=json&min_hotttnesss=0.0&max_hotttnesss=1.0&sort=hotttnesss-desc&results=1&start="
					+ start_idx + "&artist_location=" + location;

			if (jfp.setUrl(url)) {
				data_list = jfp.parser(echonest_artist_root_key,
						echonest_artist_key_name_list);

				if (data_list.size() != 0) {
					for (Map<String, Object> map : data_list)
						echonest_artist_list
								.add((HashMap<String, Object>) contents_replace(
										map, echonest_artist_key_name_list));
					System.out.println(year + " " + location + " "
							+ (start_idx + data_list.size()) + " complete");
				} else
					System.out.println(year + " " + location + " not found");
		...
		if (echonest_artist_list.size() > 0) {
			...
			// get music_list
			List<HashMap<String, Object>> echonest_music_list = new ArrayList<HashMap<String, Object>>();
			...
			List<HashMap<String, Object>> youtube_music_list = new ArrayList<HashMap<String, Object>>();
			...
			for (Map<String, Object> artist : echonest_artist_list) {
				for (int start_idx : start_index_list) {
					String echonest_url = "http://developer.echonest.com/api/v4/song/search?api_key="
							+ echonest_key
							+ "&format=json&song_max_hotttnesss=1&song_min_hotttnesss=0&&bucket=audio_summary&bucket=song_type&results=100&sort=song_hotttnesss-desc&start="
							+ start_idx + "&artist_id=" + artist.get("/id");

					if (echonest_jfp.setUrl(echonest_url)) {
						data_list = echonest_jfp.parser(
								echonest_music_root_key,
								echonest_music_key_name_list);
						int count = 0;
						...
						if (data_list.size() != 0) {
							for (Map<String, Object> map : data_list) {
								map = contents_replace(map,
										echonest_music_key_name_list);

								if (map.size() > 0)
									echonest_music_list
											.add((HashMap<String, Object>) map);
								else
									count--;
							}
				...
				for (int i = print_start_idx; i < echonest_music_list.size(); i++) {
					...
					try {
						coded_artist = new String(artist_name.getBytes(),
								"iso-8859-1");
						coded_music_name = new String(music_name.getBytes(),
								"iso-8859-1");

						String youtube_url = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=1&q="
								+ coded_artist.replace(" ", "%20")
										.replace("&", "%26")
										.replace("'", "%27")
										.replace(";", "%3B")
								+ "-"
								+ coded_music_name.replace(" ", "%20")
										.replace("&", "%26")
										.replace("'", "%27")
										.replace(";", "%3B")
								+ "&key="
						...
						if (youtube_jfp.setUrl(youtube_url)) {
							data_list = youtube_jfp.parser(
									youtube_music_url_root_key,
							...
							if (found = (data_list.size() != 0)) {
								for (Map<String, Object> map : data_list) {
									...
									if (map.size() != 0)
										...
										echonest_music_list.set(music_idx,
							...
							System.out.println(artist_name + " " + music_name
									+ (found ? "" : " not") + " found");
						} else
							return;

						System.out
								.println(youtube_music_list.size()
										+ " "
										+ echonest_music_list.size()
										+ " "
										+ (youtube_music_list.size() == echonest_music_list
												.size()));
						if ((youtube_music_list.size() % max_print == 0 || youtube_music_list
								.size() == echonest_music_list.size())
								&& youtube_music_list.size() > print_start_idx) {
							String file_name = artist_name + "-"
									+ print_start_idx / max_print + "-"
									+ location + ".txt";
							print_start_idx = setInsert(location, file_name,
									print_start_idx, max_print,
									youtube_music_list, echonest_music_list);
						}
	...
	public int setInsert(String location, String file_name, int start_idx,
			int print_max, List<HashMap<String, Object>> youtube_music_list,
			List<HashMap<String, Object>> echonest_music_list) {
		int i = 0;
		...
		for (i = start_idx; i < (start_idx + print_max > youtube_music_list
				.size() ? youtube_music_list.size() : start_idx + print_max); i++) {
			...
			if (youtube_map.size() > 0 && echonest_map.size() > 0) {
				...
				temp = temp
						.concat("'" + youtube_map.get("/id/videoId") + "', "
								+ "'" + echonest_map.get("/id") + "', ")
						.concat("'" + echonest_map.get("/title") + "', ")
						.concat("'" + echonest_map.get("/artist_name") + "', ")
						.concat("'"
								+ youtube_map
										.get("/snippet/thumbnails/high/url")
								+ "', "
								+ echonest_map.get("/audio_summary/tempo")
								+ ", ").concat("'" + location + "'");
		...
		for (String query : insert_query) {
			if (!query.equals("")) {
				((StringBuffer) sb).append("insert into musicInfo values ("
						+ query + ");\n");
				((StringBuffer) sb).append(title_list.get(insert_query
						.indexOf(query)) + ";\n");
			}
		}
	...
	public void listPrint(List<ArrayList<HashMap<String, Object>>> data,
			Map<String, Boolean> key_name) {
		for (List<HashMap<String, Object>> list : data)
			for (Map<String, Object> map : list)
				for (String key : key_name.keySet())
					System.out.println(data.indexOf(list) + " "
							+ list.indexOf(map) + " " + key + " "
							+ map.get(key));
	}
